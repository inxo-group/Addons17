...
def get_l10n_do_datetime():
    ...
    Because a user can use a distinct timezone, this method ensures that
    DR localtime stuff like auto-expire Fiscal Sequence by its date works,
    no matter server/client timezone.
    """
...
class AccountFiscalSequence(models.Model):
    ...
    required=True,
    readonly=False,     # <- antes readonly=True
    tracking=True,
)

    @api.depends("state", "fiscal_type_id", "company_id")
    def _compute_can_be_queue(self):
        for rec in self:
            if rec.state != "draft":
                rec.can_be_queue = False
                continue
            ...
            rec.can_be_queue = bool(2 > count > 0)

    @api.depends("remaining_percentage", "sequence_start", "sequence_end")
    def _compute_warning_gap(self):
        for rec in self:
            rec.warning_gap = (rec.sequence_end - (rec.sequence_start - 1)) * (
                rec.remaining_percentage / 100.0
            )

    @api.depends("sequence_end", "sequence_id.number_next_actual")
    ...
    def _compute_next_fiscal_number(self):
        for rec in self:
            if not rec.sequence_id or not rec.fiscal_type_id:
                rec.next_fiscal_number = False
                continue
            rec.next_fiscal_number = "%s%s" % (
                rec.fiscal_type_id.prefix or "",
                str(rec.sequence_id.number_next_actual).zfill(rec.sequence_id.padding or 0),
            )
    ...
    def _onchange_fiscal_type_id(self):
        ...
        if self.fiscal_type_id and self.state == "draft":
            ...
            self.sequence_start = fs.sequence_end + 1 if fs else 1

    @api.constrains("fiscal_type_id", "state", "company_id")
    def _validate_unique_active_type(self):
        for rec in self:
            if rec.state != "active" or not rec.fiscal_type_id or not rec.company_id:
                continue
            domain = [
                ("state", "=", "active"),
                ("fiscal_type_id", "=", rec.fiscal_type_id.id),
    ...
    def _validate_sequence_range(self):
        for rec in self.filtered(lambda s: s.state != "cancelled"):
            if any(value <= 0 for value in (rec.sequence_start, rec.sequence_end)):
    ...
    def name_get(self):
        return [(rec.id, "%s - %s" % (rec.name, rec.fiscal_type_id.name)) for rec in self]
    ...
    def _action_confirm(self):
        for rec in self:
            ...
            # Compat: some versions still have number_next stored; set if present.
            if "number_next" in self.env["ir.sequence"]._fields:
                sequence_vals["number_next"] = rec.sequence_start

            seq = self.env["ir.sequence"].create(sequence_vals)
            rec.write({"state": "active", "sequence_id": seq.id})
    ...
    def _action_cancel(self):
        for rec in self:
            rec.state = "cancelled"
            if rec.sequence_id:
                # Preserve internal sequence for audit purposes.
    ...
    def _expire_sequences(self):
        """
        Called from ir.cron: checks all active sequences expiration_date and
        sets state=expired if necessary.
        """
        l10n_do_date = get_l10n_do_datetime().date()
        seqs = self.search([("state", "=", "active")])
        for seq in seqs.filtered(lambda s: l10n_do_date >= s.expiration_date):
            seq.state = "expired"
    ...
    def get_fiscal_number(self):
        ...
        if (self.sequence_remaining - 1) < 1:
            self.state = "depleted"
            queued = self._get_queued_fiscal_sequence()
            if queued:
                queued._action_confirm()

        return "%s%s" % (
            self.fiscal_type_id.prefix or "",
            str(sequence_next).zfill(self.sequence_id.padding or 0),
        )
...
class AccountFiscalType(models.Model):
    ...
    prefix = fields.Char(string="Prefix", copy=False)
    ...
    def _compute_journal_type(self):
        for fiscal_type in self:
            fiscal_type.journal_type = "sale" if (fiscal_type.type or "").startswith("out") else "purchase"
    ...
    def check_format_fiscal_number(self, fiscal_number, type=""):
        ...
        if not re.match(r"^[0-9]+$", fiscal_number[3:]):
            raise ValidationError(_("After the document type, all characters must be digits from 0 to 9."))
